...
axios.interceptors.request.use(req => {
    // 如果不是去登录页面，那么就要携带token方便后台验证token的有效期
    // 注意全局守卫里面只是判断状态层是否有登录过的用户信息，用以判断是否登录了，但是没办法判断用户的登录期限是否到期，也就是token的有效期
    // 所以在强求拦截里面将用户信息里面的token加到请求信息里面，让后台判断此token是否过期，如果过期那么就强制重新登录
